...
function part1(): number {
    ...
    const _example_input = `xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))`;
    return parse(day3input);
}
...
function part2(): number {
    const day3input = Deno.readTextFileSync("input/day3.txt");
    const _example_input = `xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))`;
    ...
    while (currentIndex < input.length) {
        ...
        const slice = input.slice(currentIndex, nextIndex > 0 ? nextIndex : input.length);
        if (nextCommand === 'don\'t()') {
            dostring = dostring.concat(slice);
        }
        currentIndex = nextIndex > 0 ? nextIndex + nextCommand.length : input.length;
        currentType = nextCommand;
    }
    return parse(dostring);
}
