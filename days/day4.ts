...
function add_coordinates(
  ...
  return {
    ...
    col: coordinate1.col + coordinate2.col,
  };
}
...
function findChar(split: string[][], char: string): Coordinate[] {
  return split.reduce((accumulator: Coordinate[], current, rowIndex) => {
    return [
      ...accumulator,
      ...current.reduce(
        (lineAccumulator: Coordinate[], currentLine, colIndex) => {
          if (currentLine !== char) {
...
function search(
  ...
  if (
    input[start_coordinate.row][start_coordinate.col] !== LETTERS[letter_index]
  ) {
    return false;
  }
  ...
  return search(
    input,
    add_coordinates(start_coordinate, vector),
    vector,
    letter_index + 1,
  );
}
...
function part1(): number {
  ...
  const _example_input = `MMMSXXMASM
MSAMXMSMSA
AMXSXMAAMM
MSAMASMSMX
XMASAMXAMM
...
MXMXAXMASX`;
  ...
  const coordinates = xes.reduce<Coordinate[]>(
    (accumulator, coordinate: Coordinate) => {
      return [
        ...accumulator,
        ...SEARCH_VECTORS_DIAGONAL.reduce<Coordinate[]>(
          (inneraccumulator, vector) => {
  ...
  coordinates.forEach((coordinate) => {
    counter.set(
      ...
      (counter.get(`${coordinate.row},${coordinate.col}`) ?? 0) + 1,
    );
  });
  let result = 0;
